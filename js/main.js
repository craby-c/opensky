jQuery(function($) {
    ...
    $('#contact-us-form').submit(function() {
        ...
        form.find('.required-field').each(function() {
            ...
            if($.trim($(this).val()) === '') {
                $(this).addClass('not-valid').parent().append('<div class="error-msg">Это обязательное поле.</div>');
                hasError = true;
            } else if($(this).hasClass('email-field')) {
                var emailReg = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/;
                if(!emailReg.test($.trim($(this).val()))) {
                    $(this).addClass('not-valid').parent().append('<div class="error-msg">Вы ввели некорректный e-mail.</div>');
                    hasError = true;
                }
            }
        });
        if(!hasError) {
            ...
            $.post('contacts.php', formData, function(data) {
                ...
                form.append('<div class="success-msg">Спасибо! Мы свяжемся с Вами в ближайшее время.</div>');
            }).fail(function() {
                //form.find('.required-field').val('');
                form.append('<div class="error-msg">Произошла ошибка. Попробуйте позднее.</div>');
            });
        }               
        return false;   
    });
